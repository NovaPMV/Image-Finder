...
def content_id(path: Path) -> str:
    """A location-INDEPENDENT identity for a file, based on its actual bytes.
    This is what lets us recognize the SAME image after it's been renamed or
    moved to a different folder, so we don't re-index or regenerate its
    thumbnail.

    For speed we don't read the whole file - we hash the file SIZE plus the
    first and last 1 MB. For real-world image files that combination is
    effectively unique. The thumbnail is named from this id, so it travels with
    the file's content rather than its path."""
    CHUNK = 1024 * 1024                      # 1 MB from each end
    stat = path.stat()
    h = hashlib.md5()
    h.update(str(stat.st_size).encode())     # size is a strong, free discriminator
    try:
        ...
        # If the file can't be read, fall back to size+mtime so we still get a
        # stable-ish id rather than crashing the whole index.
        h.update(f"{int(stat.st_mtime)}".encode())
    return h.hexdigest()[:16]                # 16 hex chars is plenty here
...
def make_thumbnail_from_image(img, image: Path, thumbs_dir: Path, cid: str):
    ...
    The thumbnail file is named by CONTENT ID (cid), not the file path, so it
    stays valid if the image is later renamed or moved. If a thumbnail for this
    content already exists we reuse it instead of re-saving.
    ...
    out_path = thumbs_dir / f"{cid}.jpg"
    try:
        w, h = img.size                       # the ORIGINAL resolution (for the tag)
        if out_path.exists() and out_path.stat().st_size > 0:
            return out_path, w, h             # already made; reuse it
...
def make_thumbnail(image: Path, thumbs_dir: Path, cid: str = None):
    ...
    try:
        ...
        return None, 0, 0
    if cid is None:
        cid = content_id(image)
    return make_thumbnail_from_image(img, image, thumbs_dir, cid)
...
def save_index():
    ...
    # Make sure the thumbnails folder actually exists before writing into it. If
    # the user pointed the Thumbnails box at a new/moved location that isn't
    # there yet, create it rather than crashing with "No such file or directory".
    try:
        meta_path.parent.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        raise RuntimeError(
            f"Can't write the index to the Thumbnails folder '{meta_path.parent}'. "
            f"Make sure that folder path is correct and the drive is connected, "
            f"then click Save folders and try again. ({e})"
        )
    meta = {
        "images": STATE["images"],
        "order": STATE["order"],
...
def build_index_job():
    ...
    try:
        ...
        # PREFLIGHT: the index and thumbnails live in the Thumbnails folder. If
        # you MOVED it and the box still points at the old location (or you typed
        # a path that doesn't exist / drive not connected), bail out early with a
        # clear message instead of a cryptic file error partway through.
        if not str(thumbs_dir).strip():
            p["message"] = ("No Thumbnails folder set. Pick one, click Save "
                            "folders, then Build / update index.")
            return
        try:
            thumbs_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            p["message"] = (f"Can't use the Thumbnails folder '{thumbs_dir}'. If "
                            f"you moved it, update the Thumbnails box to its new "
                            f"location, click Save folders, then index again. ({e})")
            return
        ...
        #
        # We identify files by CONTENT ID (their bytes), not path, so an image
        # that was merely renamed or moved is recognized as already-indexed and
        # gets RE-POINTED to its new path instead of reprocessed.
        #
        # Build a map of content ids we already have -> their old key (path).
        cid_to_oldkey = {}
        for k, info in STATE["images"].items():
            c = info.get("content_id")
            if c:
                cid_to_oldkey[c] = k
        ...
        for v, sf in all_images:
            ...
            # Case 1: same path, unchanged file -> nothing to reprocess.
            if known is not None and known.get("fingerprint") == fp:
                if known.get("source") != sf:
                    known["source"] = sf
                # UPGRADE: older indexes have no content_id. Backfill it now
                # (cheap - just reads a bit of the file) so future moves are
                # recognized. The thumbnail for these older entries was named by
                # the old path-hash scheme; it keeps working at its current path.
                # If such a file is later moved, it'll be reprocessed once and
                # then be move-proof after.
                if not known.get("content_id"):
                    known["content_id"] = content_id(v)
                continue

            # Compute content id to check whether this is a moved/renamed file.
            cid = content_id(v)
            old_key = cid_to_oldkey.get(cid)

            # Case 2: this content is already indexed under a DIFFERENT path
            # (a move/rename). Re-point the existing record to the new path - no
            # thumbnail or embedding regeneration needed. We also move its entry
            # in 'order' so its existing vector row stays correctly aligned.
            if old_key is not None and old_key != key and old_key in STATE["images"]:
                rec = STATE["images"].pop(old_key)
                rec["fingerprint"] = fp        # refresh (mtime may differ after a move)
                rec["source"] = sf
                rec["content_id"] = cid
                STATE["images"][key] = rec
                # keep the vector alignment: rename this path in the order list
                for idx, opath in enumerate(STATE["order"]):
                    if opath == old_key:
                        STATE["order"][idx] = key
                        break
                cid_to_oldkey[cid] = key       # keep the map current
                repointed += 1
                continue

            # Case 3: same path but the file changed, OR a genuinely new file.
            to_process.append((v, fp, sf, cid))
        ...
        skipped = len(all_images) - total - repointed
        moved_note = f", {repointed} moved/renamed (reused)" if repointed else ""
        p.update({"done": 0, "total": total,
                  "message": f"Starting: {total} new/changed to index "
                             f"({skipped} unchanged{moved_note})"})
        ...
        def flush_batch(pending):
            ...
            for item, vec in zip(pending, vecs):
                STATE["images"][str(item["image"])] = {
                    "thumb": item["thumb"],
                    "fingerprint": item["fp"],
                    "content_id": item["cid"],   # location-independent identity
                    "width": item["w"],
                    "height": item["h"],
                    "source": item["sf"],
        ...
        for i, (image, fp, sf, cid) in enumerate(to_process):
            ...
            # 1) thumbnail (made from the already-open image, named by content id)
            thumb, w, h = make_thumbnail_from_image(img, image, thumbs_dir, cid)
            if thumb is None:
                continue

            # 2) queue this image for the next batch embedding
            pending.append({"img": img, "image": image, "fp": fp, "sf": sf,
                            "cid": cid, "thumb": str(thumb), "w": w, "h": h})
...
def build_result_item(image: str, score=None):
    ...
    # Send the content id so the grid can ask for the thumbnail from the CURRENT
    # thumbs folder (thumbs are named <content_id>.jpg). This is what makes
    # previews keep working after the thumbnails folder is renamed or moved.
    # For older records without a content id, fall back to the stored thumb path.
    return {
        "image": image,                         # original source path (for copying/opening)
        "cid": info.get("content_id", ""),      # thumbnail is <cid>.jpg in the thumbs folder
        "thumb": info.get("thumb", ""),         # legacy absolute path (fallback only)
...
@app.get("/thumb")
def get_thumb(path: str = "", cid: str = ""):
    # Preferred: serve by CONTENT ID from the CURRENT thumbs folder. Thumbnails
    # are named <cid>.jpg, so this keeps working even after the thumbs folder is
    # renamed or moved (the stored absolute path would be stale, but the current
    # folder + cid always resolves to the right file).
    if cid:
        thumbs_dir = Path(STATE["folders"].get("thumbs", "") or ".")
        p = thumbs_dir / f"{cid}.jpg"
        if p.exists():
            return FileResponse(str(p), media_type="image/jpeg")
        # fall through to the legacy path if, for some reason, the cid file isn't there
    # Legacy / fallback: serve by absolute path (older records without a cid).
    if path:
        p = Path(path)
...
function renderPage(){
  ...
  for(const item of slice){
    ...
    // Prefer requesting by content id (served from the CURRENT thumbs folder,
    // so it survives that folder being renamed/moved); fall back to the stored
    // path for older records that predate content ids.
    img.src = item.cid
      ? '/thumb?cid=' + encodeURIComponent(item.cid)
      : '/thumb?path=' + encodeURIComponent(item.thumb);
